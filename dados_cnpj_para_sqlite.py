...
#         df = pd.read_csv(arq, sep=';', header=None, names=colunas, encoding='latin1', dtype=str, na_filter=None)

#         with sqlite3.connect('rede_cnpj.db') as conn:
#             df.to_sql(nome_tabela, conn, index=False, if_exists='append', dtype={col: 'TEXT' for col in df.columns})
...
def carregaTipo(nome_tabela, tipo, colunas, chunk_size=10000):
    arquivos = list(glob.glob(os.path.join(pasta_saida, '*' + tipo)))
    for arq in arquivos:
        print(f'carregando: {arq=} em {nome_tabela}')
        print('lendo csv ...', time.asctime())
        ...
        # Ler o CSV em chunks
        with sqlite3.connect('dados-publicos\cnpj.db') as conn:
            for chunk in pd.read_csv(arq, sep=';', header=None, names=colunas, encoding='latin1', dtype=str, na_filter=None, chunksize=chunk_size):
                chunk.to_sql(nome_tabela, conn, index=False, if_exists='append', dtype={col: 'TEXT' for col in chunk.columns})
                # print(f'chunk carregado em {nome_tabela}')
...
#"D:\SINARC\rede-cnpj-master\rede_cria_tabelas\dados-publicos\cnpj.db"
...
#         # ddf['cnpj_basico'] = ddf['cnpj_basico'].astype(str)
