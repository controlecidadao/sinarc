...
Para ampliar a capacidade de pesquisa, baixe e descompacte na pasta SINARC (criada para abrigar o sistema) a versão portátil e gratuita do programa [DB Browser](https://sqlitebrowser.org/dl/) (arquivo "_DB Browser for SQLite - .zip (no installer) for 64-bit Windows_"). Com o auxílio do DB Browser é possível realizar filtros e consultas SQL diretamente nas tabelas dos bancos de dados do sistema, combinando critérios de busca que não estão disponíveis nas interfaces do SINARC e no Rede CNPJ, e chegar aos números de CNPJ que deseja explorar com o SINARC.
