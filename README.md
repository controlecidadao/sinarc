...
Para ampliar a capacidade de pesquisa por CNPJ, baixe e descompacte na pasta SINARC (pasta criada para abrigar o sistema) a versão portátil e gratuita do programa [DB Browser](https://sqlitebrowser.org/dl/) (arquivo "DB Browser for SQLite - .zip (no installer) for 64-bit Windows"). Com o auxílio do DB Browser é possível realizar consultas diretamente nos bancos de dados do sistema, combinando critérios de busca que não estão disponíveis nas interfaces do SINARC e no Rede CNPJ, e filtrar os números de CNPJ que deseja explorar com o SINARC.
