...
O SINARC captura um instante (data da disponibilização da base de dados pela Receita Federal) do processo contínuo de evolução de um sistema complexo, dinâmico e real, consistente nos registros e atualizações diárias de mais de 60 milhões de CNPJ. A expansão controlada do grafo a partir da adição de camadas sucessivas a um ou mais nós iniciais também reproduz o comportamento de um sistema complexo.
